...
namespace Revolt.Net.Commands.Attributes
{
    ...
    /// </summary>
    /// <remarks>
    ///     This attribute will override the <see cref="Readers.TypeReader"/> to be used when parsing for the
    ///     desired type in the command. This is useful when one wishes to use a particular 
    ///     <see cref="Readers.TypeReader"/> without affecting other commands that are using the same target
    ///     type.
    ///     <note type="warning">
    ///         If the given type reader does not inherit from <see cref="Readers.TypeReader"/>, an 
    ///         <see cref="ArgumentException"/> will be thrown.
    ///     </note>
    /// </remarks>
    /// <example>
    ///     In this example, the <see cref="TimeSpan"/> will be read by a custom 
    ///     <see cref="Readers.TypeReader"/>, <c>FriendlyTimeSpanTypeReader</c>, instead of the 
    ///     <see cref="TimeSpanTypeReader"/> shipped by Revolt.Net.
    ///     <code language="cs">
    ///     [Command("time")]
