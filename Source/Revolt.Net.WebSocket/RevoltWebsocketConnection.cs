...
internal sealed class RevoltWebSocketConnection
{
    ...
    internal event Func<SocketMessagePayload, Task> MessageReceived = default!;
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        while (true)
        {
            ...
            {
                Console.WriteLine($"Incoming : {JsonSerializer.Serialize(JsonNode.Parse(content), RestSerialization.Options)}");
    ...
    private Task OnMessageReceieved(string message)
    {
        ...
        return type switch
        {
            ...
            _ => MessageReceived?.Invoke(new SocketMessagePayload(type, node))
        } ?? Task.CompletedTask;
    }

    private Task OnPongEvent(JsonNode message)
    {
        var e = message.Deserialize<PongEvent>(RestSerialization.Options);
    ...
    {
        var json = JsonSerializer.Serialize(message, RestSerialization.Options);
